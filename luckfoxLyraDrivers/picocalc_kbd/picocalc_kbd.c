...
*/
	if ((ev->state == KEY_STATE_PRESSED) && (ev->scancode == 0xA2 || ev->scancode == 0xA3)) {
		...
		if (!ctx->left_shift_pressed && !ctx->right_shift_pressed) {
			// 任意 Shift 松开时，如果追踪的组合键没有松开，手动松开
...
/*
处理 esc - brk 这种组合按键触发2nd_key(brk)时因为shift没有释放导致实际触发 shift+ 2nd_key 的问题;
以及如果先松开shift，固件报告的是 1st_key “松开”导致的按键错乱问题;
*/
	//Shift + Enter 会报告为 0xd1 Insert，因此keymap里面把0xd1也映射为 Enter，先特殊处理 单独按 Insert 的情况，
...
*/
    ...
    */
		...
		g_ctx->left_shift_pressed = false;  // 初始化两个shift的状态
		g_ctx->right_shift_pressed = false; // 初始化两个shift的状态
